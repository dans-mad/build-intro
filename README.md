...
```
* To make use of this new config, instead of running `npx webpack` we're going to set up an npm script to run Webpack with the config in `webpack.config.js`. Open up `package.json`
 * Find the `scripts` section. It will probably have one child, `"test": "echo \"Error: no test specified\" && exit 1"` - you can delete that for now, although there's no harm in leaving it there.
 * We'll call our npm script `build`, because it will become our build pipeline. This means it can be run using `npm run build` (if you're like me, you'll keep forgetting the `run` in the middle and accidentally try to run `npm build`. That won't work). The _value_ for the key `build` is `webpack --config webpack.config.js` - that's the command that's run every time you use `npm run build`.
 * In later lessons we'll be adding more scripts to this section, but for now it should look like this:
 ```
"scripts": {
...
* Add a new key to `webpack.config.js` named `devServer`:
```
  devServer: {
